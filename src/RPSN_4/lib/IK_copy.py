...
def calculate_IK(input_tar, MLP_output_base, a, d, alpha):
    loss_fn_t1 = torch.nn.MSELoss()
    loss_fn_t4 = torch.nn.MSELoss()
    loss_fn_t2 = torch.nn.MSELoss()
    loss_fn_t3 = torch.nn.MSELoss()
    ...
    # if abs(theta11) > fanwei1[0]:
    #     # angle_solution = torch.tensor([(abs(t1[0]) - fanwei1[0]) * 10000], requires_grad=True)
    #     angle_solution = loss_fn_t1(torch.abs(theta11), fanwei1[0])
    #     num_Error3 += 1
    ...
    if BB**2 - 4*AA*CC >= 0:
        ...
        num_Error1_loss = (abs(BB**2 - 4*AA*CC) - torch.tensor([0.0], requires_grad=True)) * 1000
    ...
    #         the_loss_of_over = the_loss_of_over + loss_fn_t4(torch.abs(t4[i]), fanwei1[3])
    ...
    # over_index = torch.where(rad_t2 < torch.FloatTensor([-0.642788]))
    # # print(t2)
    # # print(over_index)
    # for ii in over_index[0]:
    #         the_NANLOSS_of_illegal_solution_with_num_and_Nan = the_NANLOSS_of_illegal_solution_with_num_and_Nan + \
    #                                                                     abs(abs(save_what_caused_Error2_as_Nan[ii]) - torch.tensor([0.642788])) * 100
    ...
    # print("[grads]the_NANLOSS_of_illegal_solution_with_num_and_Nan:", grads)
    # FF1.register_hook(save_grad('FF1'))
    # print("[grads]FF1:", grads)
    # GG1.register_hook(save_grad('GG1'))
    # print("[grads]GG1:", grads)
    ...
    #         # the_loss_of_over = loss_fn_t2(torch.abs(t2[i]), fanwei1[1]) + torch.tensor([0.0], requires_grad=True)
    #         # break
    ...
    # print("[grads]t3:", grads)
    # FF1.register_hook(save_grad('FF1'))
    # print("[grads]FF1:", grads)
    # GG1.register_hook(save_grad('GG1'))
    # print("[grads]GG1:", grads)

    FF = torch.FloatTensor([FF1, FF1, FF2, FF2, FF3, FF3, FF4, FF4])
    ...
    for index_ok_t2_ii in index_ok_t2[0]:
        if not torch.abs(t3[index_ok_t2_ii]) <= fanwei1[2]:
            # diff = FF[index_ok_t2_ii]*cos(fanwei1[2] + t2[index_ok_t2_ii]) - GG[index_ok_t2_ii]*sin(fanwei1[2] + t2[index_ok_t2_ii])
            # diff = torch.FloatTensor(diff)
            # the_loss_of_over = the_loss_of_over + torch.tensor([diff - 0], requires_grad=True) * 1000
